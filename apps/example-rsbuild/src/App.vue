...
/**
 * LYStack — 构建工具无关的企业级 Vue3 Monorepo 底座
 *
 * 本页面集中演示 @repo/shared、@repo/services、@repo/ui
 * 三个核心包的用法，同时验证 Rsbuild 构建链路完整。
 */
...
/**
 * 模拟一次 API 调用演示 services 层已就绪。
 * 由于没有真实后端，此调用会走到网络错误 —— 这正是演示目的：
 * 证明 apiService 已正确装配、拦截器链路畅通。
 * 打开 DevTools Network / Console 即可看到请求发出。
 */
  ...
  <div class="app-shell">
    ...
    <header class="hero">
      ...
      <div class="hero__meta">
        <span class="tag tag--build">{{ buildTool }} SPA</span>
    ...
    <footer class="footer">
      <p>
        LYStack · <a href="https://github.com" target="_blank" rel="noopener">GitHub</a> · 构建于 {{ buildTool }}（{{
          buildMode
        }}）
      </p>
      <p class="footer__hint">打开 DevTools Console 查看 bootstrap 装配日志与 API 调用详情</p>
    </footer>
  </div>
</template>
